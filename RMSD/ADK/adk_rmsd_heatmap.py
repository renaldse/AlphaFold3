...
# ── Config ─────────────────────────────────────────────────────────────────────
CIF_BASE = "/mnt/gs21/scratch/renaldse/AlphaFold3/inputs/structures"
OUT_DIR  = "/mnt/gs21/scratch/renaldse/AlphaFold3/RMSD/ADK"
...
# ── Structure paths and labels ─────────────────────────────────────────────────
all_labels = ["ADK_open", "ADK_closed"]
all_paths  = [
    f"{CIF_BASE}/ADK_open.cif",
    f"{CIF_BASE}/ADK_closed.cif",
]
n = len(all_paths)
...
def ca_rmsd(ref, mob):
    n_atoms = min(len(ref), len(mob))
    sup = Superimposer()
    sup.set_atoms(ref[:n_atoms], mob[:n_atoms])
...
def plot_heatmap(matrix, labels, title, cbar_label, out_path):
    fig, ax = plt.subplots(figsize=(5, 4))

    cmap = mcolors.LinearSegmentedColormap.from_list("white_blue", ["white", "blue"])
    vmin, vmax = 0.0, max(np.nanmax(matrix), 0.1)
    ...
    ax.set_xlabel("Structure (X)", fontsize=12)
    ax.set_ylabel("Structure (Y)", fontsize=12)
...
for mode in ['calpha', 'allatom']:
    ...
    mode_label = "Cα" if mode == 'calpha' else "All-Atom"
    plot_heatmap(
        matrix, all_labels,
        title=f"ADK Open vs Closed — {mode_label} RMSD\n(no iterative rejection)",
        cbar_label=cbar_label,
        out_path=f"{OUT_DIR}/adk_rmsd_{mode}_heatmap.png",
    )
