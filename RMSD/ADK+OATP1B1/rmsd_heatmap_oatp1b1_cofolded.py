...
# Columns – cofolded samples 1–4
COL_LABELS = ["Cofolded\nSample-1", "Cofolded\nSample-2",
               "Cofolded\nSample-3", "Cofolded\nSample-4"]
COL_PATHS  = [f"{BASE}/{SEED}_sample-{i}/model.cif" for i in range(1, 5)]

# Rows – sample-0, OATP1B1 outward, OATP1B1 inward
...
for r, (rlabel, rca) in enumerate(zip(ROW_LABELS, row_cas)):
    for c, (clabel, cca) in enumerate(zip(COL_LABELS, col_cas)):
        ...
        rmsd_matrix[r, c] = val
        print(f"  {rlabel.replace(chr(10),' ')} vs {clabel.replace(chr(10),' ')}: {val} Å")
...
ax.set_yticklabels([l.replace("\n", "\n") for l in ROW_LABELS], fontsize=11)
...
plt.tight_layout()
out_path = "/mnt/gs21/scratch/renaldse/AlphaFold3/RMSD/rmsd_heatmap_oatp1b1_cofolded.png"
