...
Run:
    python mantel_af3_approachB.py --seq seq15
    python mantel_af3_approachB.py --seq seq13 --mode ca
    python mantel_af3_approachB.py --seq seq5  --mode allatom
"""
...
N_CLUSTERS          = 10
SAMPLES_PER_CLUSTER = 10   # 2 seeds x 5 samples
N_TOTAL             = N_CLUSTERS * SAMPLES_PER_CLUSTER  # 100
N_PERMUTATIONS      = 9999
...
def find_cluster_output_dir(seq, cluster_idx):
    pattern = str(OUTPUTS_DIR / f"{seq}_cluster{cluster_idx}_templated*")
...
def collect_sample_cifs(seq, cluster_idx):
    ...
    cifs    = sorted(glob.glob(str(inner / "seed-*_sample-*" / "model.cif")))
...
def build_sample_matrix_B(mode, sample_atoms):
    print(f"  Building 100x100 sample matrix ({mode}) ...")
...
def plot_matrices(tmpl_mat, samp_mat, seq, mode, r, p, out_path):
    ...
    fig.suptitle(
        f"{seq.upper()} - Mantel Test (Approach B: 100x100)\n"
...
def run(seq, mode):
    ...
    plot_matrices(tmpl_mat_100, samp_mat, seq, mode, r, p,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Mantel test (Approach B: 100x100) for AF3 template bias")
    parser.add_argument("--seq",  required=True,
                        help="Sequence name, e.g. seq15, seq13, seq5")
    parser.add_argument("--mode", default="allatom",
                        choices=["ca", "allatom"],
                        help="RMSD mode (default: allatom)")
    args = parser.parse_args()
    run(args.seq, args.mode)
