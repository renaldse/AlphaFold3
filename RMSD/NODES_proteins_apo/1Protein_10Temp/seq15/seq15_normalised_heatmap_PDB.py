...
# ── PDB references (shown at top of y-axis) ────────────────────────────────────
PDB_REFS = [
    ("4KSO (monomer)",  f"{CIF_BASE}/seq15_PDB_cleaned_4KSO_monomer.cif"),
    ("5JWO (multimer)", f"{CIF_BASE}/seq15_PDB_cleaned_5JWO_multimer.cif"),
    ("5JWR (multimer)", f"{CIF_BASE}/seq15_PDB_clustered_5JWR_multimer.cif"),
]
N_PDB = len(PDB_REFS)
...
def plot_heatmap(matrix, row_labels, col_labels, title, cbar_label, out_path, n_pdb_rows):
    ...
    # Horizontal divider between PDB refs and cluster rows
    ax.axhline(y=n_pdb_rows - 0.5, color='black', linewidth=1.8, linestyle='-')
...
for mode in ['calpha', 'allatom']:
    ...
    # ── Load PDB references ────────────────────────────────────────────────────
    print("  Loading PDB references...")
    pdb_ref_atoms = []
    pdb_row_labels = []
    for name, path in PDB_REFS:
        print(f"    {name}: {path}")
        pdb_ref_atoms.append(get_atoms(path))
        pdb_row_labels.append(f"PDB {name}")

    # ── Load all 10 cluster CIF references ────────────────────────────────────
    ...
    # ── Build row labels: PDB rows first, then cluster rows ───────────────────
    cluster_row_labels = [f"Cluster {i} CIF" for i in range(10)]
    row_labels = pdb_row_labels + cluster_row_labels
    n_rows = len(row_labels)   # 13
    ...
    for c, (col_path, home_cid) in enumerate(zip(col_paths, col_home)):
        ...
        try:
            ...
            # PDB reference rows (top N_PDB rows)
            for r, ref_atoms in enumerate(pdb_ref_atoms):
                numerator = rmsd_fn(ref_atoms, mob)
                matrix[r, c] = round(numerator / denom, 4)

            # Cluster rows (rows N_PDB onward)
            for i in range(10):
                numerator = rmsd_fn(cif_refs[i], mob)
                matrix[N_PDB + i, c] = round(numerator / denom, 4)
