...
# ── PDB references (shown at top/left of both axes) ───────────────────────────
PDB_REFS = [
...
def plot_heatmap(matrix, labels, title, cbar_label, out_path, n_pdb=3):
    ...
    # Dividing lines after the PDB reference rows/cols
    ax.axhline(y=n_pdb - 0.5, color='black', linewidth=1.8, linestyle='-')
    ax.axvline(x=n_pdb - 0.5, color='black', linewidth=1.8, linestyle='-')
...
cif_paths      = [f"{CIF_BASE}/seq15_cluster_repr_{i}.cif" for i in range(10)]
 
# PDB references first (top/left), then clusters
pdb_labels = [name for name, _ in PDB_REFS]
pdb_paths  = [path for _, path in PDB_REFS]
 
all_labels = pdb_labels + cluster_labels
all_paths  = pdb_paths  + cif_paths
n          = len(all_paths)   # 13
...
for mode in ['calpha', 'allatom']:
    ...
    plot_heatmap(
        matrix, all_labels,
        title=f"Seq15 Cluster CIF vs CIF — {cbar_label.split(' (')[0]}\n(10 cluster representatives + 4KSO, 5JWO & 5JWR PDB references)",
        cbar_label=cbar_label,
        out_path=f"{OUT_DIR}/seq15_cif_vs_cif_{mode}_heatmap.png",
        n_pdb=N_PDB,
    )
