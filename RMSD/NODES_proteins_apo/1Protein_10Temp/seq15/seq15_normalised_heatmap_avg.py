...
OUT_DIR      = "/mnt/gs21/scratch/renaldse/AlphaFold3/RMSD/NODES_proteins_apo/1Protein_10Temp/seq15"
...
for mode in ['calpha', 'allatom']:
    ...
    for c, (home_cid, seed_label, sample_paths) in enumerate(col_meta):
        ...
        # Collect per-sample normalised values across all rows (11 rows: PDB + 10 clusters)
        ...
        for s_idx, path in enumerate(sample_paths):
            try:
                ...
                # Row 0: PDB reference
                per_sample_norm[0, s_idx] = rmsd_fn(pdb_ref, mob) / denom
        ...
        # Average the normalised values across samples (ignoring nan)
    ...
    plot_heatmap(
        ...
        title=(
            f"Seq15 — Avg Normalised {'Cα' if mode == 'calpha' else 'All-Atom'} RMSD Heatmap\n"
