...
def plot_heatmap(matrix, labels, title, cbar_label, out_path, n_clusters):
    ...
    # Draw a dividing line to visually separate the PDB reference row/column
    ax.axhline(n_clusters - 0.5, color='red', linewidth=1.5, linestyle='--')
    ax.axvline(n_clusters - 0.5, color='red', linewidth=1.5, linestyle='--')
...
cif_paths      = [f"{CIF_BASE}/seq5_cluster_repr_{i}.cif" for i in range(10)]

# Append PDB reference as the last entry
all_labels = cluster_labels + ["1YOB (PDB)"]
all_paths  = cif_paths + [PDB_REF]
n          = len(all_paths)   # 11
...
for mode in ['calpha', 'allatom']:
    ...
    plot_heatmap(
        matrix, all_labels,
        title=f"Seq5 Cluster CIF vs CIF — {cbar_label.split(' (')[0]}\n(10 cluster representatives + 1YOB PDB reference)",
        cbar_label=cbar_label,
        out_path=f"{OUT_DIR}/seq5_cif_vs_cif_{mode}_heatmap.png",
        n_clusters=10
    )
